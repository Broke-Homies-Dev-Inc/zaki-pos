  ...
  switch (order.order_type) {
    ...
    case 'take_away':
      if (order.take_away_method === 'car') {
        return (
          <span className="text-gray-700">
            Car: <span className="font-medium">{order.car_details || 'N/A'}</span>
          </span>
        );
      }
                ...
                orders.map((order) => (
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {order.mobile_number || `#${order.order_number}`}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                      {order.waiter_name
                        ? order.waiter_name
                        : <span className="text-gray-400 italic">No waiter</span>}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span
                        className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${getStatusBadge(
                          order.status
                        )}`}
                      >
                        {order.status}
