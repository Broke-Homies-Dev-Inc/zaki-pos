...
export function CreateOrderModal({ onClose }: { onClose: () => void }) {
                ...
                return prevCart.map((item) =>
                    item.menu_item_id === menuItem.id
                        ? {
                              ...
                              total_price:
                                  (item.quantity + 1) * Number(item.unit_price),
                          }
                        : item
                );
            }
    ...
    const handleSubmit = async () => {
        ...
        // If failed, the error is already shown in createOrder via alert
    };
    ...
    useEffect(() => {
        const fetchByPhone = async () => {
            ...
            try {
                ...
                if (res && res.data) {
                    // populate name from existing customer
                    if (res.data.name) setCustomerName(res.data.name);
                }
            } catch (err) {
                // ignore 404 (customer not found) and other errors silently
            }
        };

        // Debounce a bit to avoid hammering API while typing
    ...
    const DineInOptions = () => {
        // Get list of all tables with their availability status
            ...
            <div className="mt-2">
                ...
                >
                    ...
                    {tableLayout.map((floor) => {
                        ...
                        const availableInFloor = floorTables.filter(
                            (t) => t.isAvailable
                        ).length;
                ...
                {availableCount === 0 && (
                    <p className="text-sm text-red-600 mt-1">
                        ...
                        table to become available or choose a different order
                        type.
                    </p>
                )}
                {!selectedTable && availableCount > 0 && (
                    <p className="text-sm text-gray-500 mt-1">
                        Available tables shown in black, unavailable tables
                        greyed out
                    </p>
                )}
                {availableCount > 0 && (
                    <div className="text-xs text-gray-600 mt-2 space-y-1">
                        <p>• 👥 Occupied - Table has active order</p>
                        <p>• 🧹 Cleaning - Being prepared (2 min)</p>
                        <p>• 🧾 Bill Printed - Awaiting payment</p>
                    </div>
                )}
            </div>
        );
                            ...
                            <div className="space-y-3">
                                ...
                                {cart.length === 0 && (
                                    <p className="text-gray-500 text-center py-4">
                                        Select items from the menu to start an
                                        order.
                                    </p>
                                )}
                            </div>
